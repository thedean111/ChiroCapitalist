Shader "Color Studio/ColorWheel"
{
	...
	inline float3 HUEtoRGB(in float H) {
  	...
  	inline float4 DrawDot(float4 color, float2 uv, float4 keyData) {
		...
		return lerp(color, 1.0 - dotColor, saturate((0.0001 / abs(d - dotWidth)) * cursor * 2.0));
	}
		...
		v2f vert (appdata v)
			...
			half4 frag (v2f i) : SV_Target {
				...
				if (d > 0.5*0.5) return 0;
					...
					float sat = 1.0;
					float alpha = light < 0.32 || (light > 0.34 && light < 0.36);
					if (light < 0) {
						float2 h0 = float2(0,999);

                        UNITY_UNROLL
                        for (int k=0;k<MAX_KEY_COLORS;k++) {
                            // Skip the last key color (lightness dot) from inner hue picking
                            if (k < _KeyColorsCount - 1)
                            {
						...
						hue = h0.x;
						// inner area: use global palette saturation & lightness
						sat = saturate(_Saturation);
						light = saturate(_Lightness);
					}  else if (light > 0.34) {
                        // outer ring: use PRIMARY hue & saturation, vary lightness vertically (top=0, bottom=1)
                        // derive primary saturation from primary key position radius within middle ring
                        float2 center = float2(0.5, 0.5);
                        float dkey = length(_Key0Pos - center);
                        float lkey = (dkey - _CenterWidth) / (1.0 - _CenterWidth);
                        float primarySat = saturate(lkey / 0.32);
                        hue = _Key0Hue;
                        sat = primarySat;
                        light = saturate(0.5 - dd.y);
                    }  else if (light < 0.32) {
						// middle ring: map radial position to saturation 0..1, fixed lightness
						sat = saturate(light / 0.32);
						light = lerp(0.5, 0, saturate(0.002 / abs(light - 0.32)) * 0.5);

					}
					float4 color = float4(HSLtoRGB(float3(hue,sat,light)), alpha);
					...
					for (int k = 0; k < MAX_KEY_COLORS; k++) {
						if (k<_KeyColorsCount) {
							color = DrawDot(color, i.uv, _KeyColorsData[k]);
						}
					}
