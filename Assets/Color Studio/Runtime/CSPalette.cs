...
namespace ColorStudio {
    ...
    public partial class CSPalette : ScriptableObject {
        ...
        public float primaryAngle => keyColors[0].angle;
        public float primaryHue => keyColors[0].hue;
        public float primarySaturation => saturation;
        public float primaryLightness => lightness;
        public Color primaryColor => keyColors[0].color;
        public Color complementary1Color => keyColors[1].color;
        public Color complementary2Color => keyColors[2].color;
        public Color complementary3Color => keyColors[3].color;
        ...
        public void ConfigurePalette(ColorScheme scheme, Color primaryColor, float splitAmount = 0.6f) {
            ...
            HSLColor hsl = ColorConversion.GetHSLFromRGB(primaryColor.r, primaryColor.g, primaryColor.b);
            SetKeyColor(0, KeyColorType.Primary, hsl.h * PI2);
            SetSchemeKeyColors();
        }
        ...
        public void SetPrimaryAngle(float angle) {
            SetKeyColor(0, KeyColorType.Primary, angle);
        }

        public void SetKeyColor(int index, KeyColorType keyColorType, float angle) {
            float hue = ((angle % PI2 + PI2) % PI2) / PI2;
            keyColors[index].angle = angle;
            keyColors[index].hue = hue;
            keyColors[index].type = keyColorType;
            keyColors[index].color = ColorConversion.GetColorFromHSL(hue, this.saturation, this.lightness);
        ...
        public void Load(CSPalette otherPalette) {
            ...
            lightness = otherPalette.lightness;
        ...
        public Color[] BuildPaletteColors() {
            ...
            int totalRows = Mathf.Max(shades, 1); // at least 1 row
            int totalColorCount = colorsCount * totalRows;
            if (paletteColors == null || paletteColors.Length != totalColorCount) {
                paletteColors = new Color[totalColorCount];
            }
            int i = 0;

            // New distribution: center row matches key color exactly; rows below are darker, above are lighter.
            int darkRows = Mathf.CeilToInt((totalRows - 1) / 2f);
            int lightRows = Mathf.Max(totalRows - 1 - darkRows, 0);
            int centerRow = darkRows; // 0-based row index

            // Bottom (darker) rows
            for (int row = 0; row < centerRow; row++) {
                // k from 1 (closest to center) .. darkRows (farthest)
                int k = centerRow - row;
                float tNorm = (k - 0.5f) / Mathf.Max(darkRows, 1);
                float t = Mathf.Lerp(lightness, minBrightness, tNorm);
            ...
            // Center row: originals as-is (temperature already applied in BuildHueColors)
            for (int c = 0; c < colorsCount && centerRow < totalRows; c++, i++) {
                paletteColors[i] = colors[c];
            }

            // Top (lighter) rows
            for (int row = centerRow + 1; row < totalRows; row++) {
                int k = row - centerRow; // 1..lightRows
                float tNorm = (k - 0.5f) / Mathf.Max(lightRows, 1);
                float t = Mathf.Lerp(lightness, maxBrightness, tNorm);
                for (int c = 0; c < colorsCount; c++, i++) {
                    paletteColors[i] = ColorConversion.GetColorFromRGBSL(colors[c].r, colors[c].g, colors[c].b, saturation, t);
                }
            }
